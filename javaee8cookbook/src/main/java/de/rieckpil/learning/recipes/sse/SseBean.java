package de.rieckpil.learning.recipes.sse;
...
public class SseBean implements Serializable {
    ...
    public void sendEvent() throws URISyntaxException, InterruptedException {
        ...
        WebTarget target = client.target(URI.create("http://localhost:9080/"));
                Response response;
        response = target.path("resources/serverSentService/start")
                .request()
        ...
        for (int i = 0; i < countClient; i++) {
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Message sent to client " + (i + 1) + ": " + message));
        }
    }
