...
class GreetingControllerTest {
  ...
  private BlockingQueue<WsMessage> blockingQueueObject = new ArrayBlockingQueue(1);
  ...
  public void testWebSocketsWithObject() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    session.subscribe("/topic/objects", new StompFrameHandler() {
      ...
      @Override
      public void handleFrame(StompHeaders headers, Object payload) {
        blockingQueueObject.offer((WsMessage) payload);
        System.out.println("Response: " + payload);
      }
    });

    WsMessage wsMessage = new WsMessage();
    wsMessage.setMessage("Mike");
    session.send("/app/object", wsMessage);

    WsMessage payload = blockingQueueObject.poll(1, SECONDS);
